...
class Crawler:
    ...
    def download_url(
        ...
        log_FileFuncLine(f"local_relative_path = {local_relative_path}")
        log_FileFuncLine(f"download_dir = {self.download_dir}")
            ...
            for e in elements:
                ...
                if href_raw is not None:
                    ...
                    if re.match(r'^https?://', href_raw, re.IGNORECASE):
                        ...
                        # this href is a relative path, we need to convert it to an absolute path
                        # so that we can download it.
                        host_url = get_host_url(url)
                        if host_url is None:
                            raise RuntimeError(f"can't get host url from {url}")
                        href_full = get_host_url(url) + href_raw
