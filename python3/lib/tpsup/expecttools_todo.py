...
def init_child(cmd, method='expect', **opt):
    # print(f"init_child: cmd={cmd}, method={method}, opt={opt}")
    if method == 'expect':
        # child = pexpect.spawn("sftp localhost")
        # AttributeError: module 'pexpect' has no attribute 'spawn'
        # https://pexpect.readthedocs.io/en/stable/overview.html#pexpect-on-windows
        # child = pexpect.popen_spawn.PopenSpawn("sftp localhost")
        child = pexpect.popen_spawn.PopenSpawn("C:/Program Files/Git/bin/bash.exe")
        # sleep 5 seconds to wait for the child to start
        time.sleep(2)
        child.sendline(cmd)
        # child.method = method
        # send_to_child(cmd, child)
    # elif method == 'pty':
    #     child = ptyprocess.PtyProcessUnicode.spawn("bash")
