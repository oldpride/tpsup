...
our_cfg = {
    ...
    'usage_example': f'''
    {tpsup.seleniumtools_old.diagram}
...
def pre_batch(all_cfg, known, **opt):
    ...
    tpsup.seleniumtools_old.pre_batch(all_cfg, known, **opt)
...
def code(all_cfg, known, **opt):
    ...
    if run_js:
        locator_chain2 = tpsup.seleniumtools_old.locator_chain_to_locator_chain_using_js(locator_chain, trap=trap, debug=debug)
    ...
    followEnv = tpsup.locatetools_old.FollowEnv(str_action=tpsup.seleniumtools_old.locate, 
                                            dict_action=tpsup.seleniumtools_old.locate_dict,
                                            **opt)
...
def parse_input_sub(input: Union[str, list], all_cfg: dict, **opt):
    ...
    if re.match(r'locators$', input[0]):
        for line in tpsup.locatetools_old.get_defined_locators(locate_func=tpsup.seleniumtools_old.locate):
            print(line)
        exit(0)

    if re.match(r'(d|download_chrome)driver$', input[0]):
        version = input[1] if len(input) > 1 else None
        tpsup.seleniumtools_old.download_chromedriver(driver_version=version)
        exit(0)

    if re.match(r'check_setup$', input[0]):
        tpsup.seleniumtools_old.check_setup(compareVersion=1)
        exit(0)
