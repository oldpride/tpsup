...
export TPSUP=$(cd "$TP_BASH_SOURCE_DIR"; pwd -P) || return
...
freshenv () {
   local VAR
   for VAR in MANPATH PERL5LIB LD_LOAD_PATH LD_LIBRARY_PATH PYTHONPATH
   do
       unset $VAR
   done
...
"
   ...
   do
      case "$o" in
         d) DEBUG=Y; FLAG="";;
         *) echo "unknow switch. ${FUNCNAME[1]}. $USAGE">&2; return 1;;
      esac
   done
   ...
   [ $DEBUG = Y ] && ( echo ""; echo "reduce() is called by '${FUNCNAME[1]}'" )
   ...
   if [ "X$TP_REDUCE_DISABLE" = "XY" ]; then
      ...
      [ $DEBUG = Y ] && ( echo ""; echo "skipped reduce() because TP_REDUCE_DISABLE=$TP_REDUCE_DISABLE" )
      return
   fi
