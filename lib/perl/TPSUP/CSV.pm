...
sub query_csv2 {
   ...
   croak "please change InputHashArray to \$opt->{InputType} = 'HashArray'" if $opt->{InputHashArray};
   croak "please change InputArrayArray to \$opt->{InputType} = 'ArrayArray'" if $opt->{InputArrayArray};
   croak "please change InputStructuredHash to \$opt->{InputType} = 'StructuredHash'" if $opt->{InputStructuredHash};
   ...
   if (!$opt->{ReturnType} || $opt->{ReturnType} eq 'StructuredHash') {
      ...
      my $tmp;
      if ( $opt->{SortKeys} ) {
         $tmp = sort_HashArray_by_keys($ref3->{array}, $opt->{SortKeys}, $opt);
      } else {
         $tmp = $ref3->{array};
      }
...
sub render_csv {
   ...
   croak "unsupport $row_type=$row_type" if $row_type ne 'ARRAY' && $row_type ne 'HASH';
