...
sub get_fixed_records {
   ...
   if (  $opt->{FixedRecordInputType}
      && $opt->{FixedRecordInputType} eq 'string' )
   {
...
# 1. use regex
   ...
   {
      my @exclude_qrs;
      if ( $opt->{FixedRecordExcludePatterns}
         && @{ $opt->{FixedRecordExcludePatterns} } )
      {
      ...
      my @match_qrs;
      if ( $opt->{FixedRecordMatchPatterns}
         && @{ $opt->{FixedRecordMatchPatterns} } )
      {
   ...
   if ( !$template ) {
      ...
      if ( $opt->{FixedRecordMatchExps} && @{ $opt->{FixedRecordMatchExps} } ) {
         @$matchExps = map { TPSUP::Expression::compile_exp( $_, $opt ) }
           @{ $opt->{FixedRecordMatchExps} };
      }

      my $excludeExps;
      if ( $opt->{FixedRecordExcludeExps}
         && @{ $opt->{FixedRecordExcludeExps} } )
      {
...
sub main {
   my $string =
"Joan        Smith       33  135 25 Elms Street    Joe         Spinsak     46  195 6 5th Ave         Stephen     Kierbarsky  9   60  125 Main Street   ";

   print "\n---------- test split -----------------\n";
   my $records =
     get_fixed_records( $string, 50, { FixedRecordInputType => 'string' } );
   ...
   print "\n---------- test unpack -----------------\n";
   $records = get_fixed_records(
      $string, 50,
      {
         FixedRecordInputType => 'string',
