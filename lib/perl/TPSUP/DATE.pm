...
sub get_yyyymmdd_by_yyyymmdd_offset {
   ...
   my $yyyymmdd2 = `date -d "$yyyymmdd1 $offset day" '+%Y%m%d'`; chomp $yyyymmdd2;
   return $yyyymmdd2;
}
...
sub epoc_to_yyyymmddHHMMSS {
   ...
   my $yyyymmddHHMMSS = sprintf('%4d%02d%02d%02d%02d%02d', 
...
sub get_new_yyyymmddHHMMSS {
   ...
   } elsif ( my @c = ( $old_yyyymmddHHMMSS =~ 
        /^([12][09]\d{2})([^\d])(\d{2})(.)(\d{2})(.)(\d{2})(.)(\d{2})(.)(\d{2})(.*)/)
      ) {
      $old_t = "$c[0]$c[2]$c[4]$c[6]$c[8]$c[10]";
      my $tail  = defined($c[13]) ? $c[13] : '';
      $out_format =  '%4d' . $c[1] . '%02d' . $c[3] . '%02d' . $c[5]  . 
                    '%02d' . $c[7] . '%02d' . $c[9] . '%02d' . $c[11] . $tail ;
   } else {
   ...
   my $new_yyyymmddHHMMSS = sprintf($out_format, 
                                    1900+$year, $mon+1, $mday,
                                         $hour, $min,   $sec,
                                   );
         ...
         for my $yyyymmdd (@holidays) {
            ...
            if ($yyyymmdd =~ /$yyyymmdd_pattern/) {
               ...
               if ($last_holiday && $yyyymmdd <= $last_holiday ) {
                  croak "$HolidaysCsv row $row_count item $item_count '$yyyymmdd' <= last one '$last_holiday', out of order";
               }
...
sub get_tradeday_by_exch_begin_offset {
   ...
   my $begin_weekday_pos = binary_search_numeric($begin, $weekdays, 0, scalar(@$weekdays)-1, {WhenInBetween=>$ChooseBigger});
...
sub main {
   ...
   print "get_tradeday_by_exch_begin_offset('NYSE', '20200905', 0, {OnWeekend=>'next'}) = ",
   ...
   print "get_tradeday_by_exch_begin_offset('NYSE', '20200907', 0, {OnWeekend=>'next'}) = ",
   ...
   print "get_tradeday_by_exch_begin_offset('NYSE', '20200907', 1, {OnWeekend=>'next'}) = ",
   ...
   print "get_seconds_between_yyyymmddHHMMSS('2021-10-21 07:01:02.513447', '2021-10-21 03:01:02.000000') = ",
