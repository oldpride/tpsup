...
sub get_keys_in_uppercase {
   ...
   if ($opt->{AliasMap}) {
      ...
      for my $a (keys(%$alias_map)) {
         my $v = $alias_map->{$a};
         if (!$seen->{uc($v)}) {
            print "'$v' is used in alias map but not seen in original keys even if case insensitive. seen keys = " . Dumper($seen);
            exit 1;
         } 
...
sub resolve_vars_array {
   ...
   my $type = ref $vars;
   croak "vars type is not ARRAY. vars = " . Dumper($vars) if ! ($type && $type eq 'ARRAY');
...
my $method_syntax = {
   ...
   section => { required => [qw(log ExtractPatterns)],
                optional => [qw( PreMatch   PreExclude
                                 PostMatch PostExclude
                                 BeginPattern EndPattern
                                 KeyAttr KeyDefault)],
              },
      path => { required => [qw(paths)],
...
sub extract_from_fh {
   ...
   my $CompiledMatch;
   my $CompiledExclude;
   if (defined($opt->{MatchPattern}))   { $CompiledMatch   = qr/$opt->{MatchPattern}/   }
   if (defined($opt->{ExcludePattern})) { $CompiledExclude = qr/$opt->{ExcludePattern}/ }
...
sub apply_key_pattern {
   ...
   for my $k (@needed_keys) {
      confess "'{{pattern::$k}}' in '$string' but '$k' is not defined in key_pattern=" .
         Dumper($key_pattern) if !$key_pattern->{$k};
   }

   for my $k (@needed_keys) {
      my $substitue = defined $known{$k} ? $known{$k} : $key_pattern->{$k}->{pattern};
...
sub apply_csv_filter {
   ...
   my $StructuredHashArray
       = query_csv2({columns=>\@headers, array=>\@hashes}, # these are in global buffer
...
sub process_path {
   ...
   # $file = resolve_scalar_var_in_string($file, {%known, %vars});
   my @resolved_paths = map {resolve_scalar_var_in_string($_, {%known, %vars})} @$paths;
...
sub update_knowledge_from_rows {
   ...
   for my $k (keys %$cfg) {
      ...
      my $condition = defined($kc->{condition}) ? $kc->{condition} : $kc->{update_knowledge};
      ...
      if (defined($code)) {
         ...
         update_knowledge($k, $new_value, {KeyConfig=>$kc}) if defined $new_value;
      }
   }
}
...
sub parse_cfg {
   ...
   if (exists $our_cfg->{key_pattern}) {
      $our_cfg->{key_pattern} = unify_hash($our_cfg->{key_pattern}, 'pattern');
   } else {
      $our_cfg->{key_pattern} = {};
   }
   ...
   my @allowed_keys = get_keys_in_uppercase($cfg_by_entity,
                                         {
                                            AliasMap=>$alias_map,
...
sub set_all_cfg {
   ...
   if (!$cfg_type) {
      ...
      croak "unknown cfg type=$cfg_type, expecting file name (string) or HASH. given_cfg = "
         . Dumper($given_cfg) ;
   }
}
...
sub trace {
   ...
   my $parsed_input = parse_input($input, 
   ...
   for my $k (qw(YYYYMMDD)) {
      update_knowledge($k, $parsed_input->{$k}) if defined $parsed_input->{$k};
   }
   ...
   my @trace_route_entities = grep {!$SkipTrace->{$_}} get_keys_from_array($trace_route, 'entity');
...
sub main {
   ...
   trace("$ENV{TPSUP}/scripts/tptrace_test_var.cfg", ['tradeid=trd001'], 
       {
          SkipTrace=>'all',
          SkipEntry=>1,
          verbose=>0,
   ...
   trace("$ENV{TPSUP}/scripts/tptrace_test.cfg", ['sec=IBM', 'sendercomp=BLKC', 'orderqty=4,500', 'yyyymmdd=20211129', 'tradeqty=600'], 
         #{verbose=>1}
        ); 
