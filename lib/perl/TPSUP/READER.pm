...
sub new {
   ...
   $self->{context_pre} = [];
   $self->{context_max} =
     defined $self->{opt}->{context_max} ? $self->{opt}->{context_max} : 3;
...
sub context_pre {
   ...
   if ( $context_max > 0 ) {
      for (
         my $i = $context_idx - $context_max + 1 ;
         $i < $context_idx + 1 ;
         $i++
        )
      {
...
# $line2 =~ s:(err|error|fatal|severe|exception)(\s|_|-)*(Msg|Mesg|Message)?[s]?\s*(are\s*|is\s*|=\s*)?(0\b|\[\s*\])::ig;
      $line2 =~
s:(err|error|fatal|severe|exception)(\s|_|-)*(Msg|Mesg|Message)?[s]?\s*((are|is|=)\s*)?(0\b|\[\s*\])::ig;
      ...
      $line2 =~
s:if\s*(I|it|you|they|she|he|there)\s*(has|have|had|is|are)(\s*a|\s*an)?\s*(error|fatal|severe|exception)::ig;
