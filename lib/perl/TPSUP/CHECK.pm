...
#use DBI;
use Data::Dumper;
use TPSUP::UTIL qw(get_tmp_file get_user);
...
sub chkcmd {
   ...
   for my $attr (qw( ExpectedStdout   ExpectedStderr
   ...
   my $stamp = sprintf("%4d%02d%02d-%02d%02d%02d", $year, $mon+1, $mday, $hour, $min, $sec);
   ...
   if (defined $check->{day_of_week}) {
      ...
      if (! grep {$wday eq $_} @wdays) {
         $ret->{stauts} = "OK: NOT_IN_TIME_RANGE: day_of_week=$check->{day_of_week}, today=$wday";
         return $ret;
      }
   }
   ...
   if (defined $check->{exclude_days}) {
      ...
      if (grep {"$day" eq $_} @exclude_days) {
         $ret->{stauts} = "OK: IN_E:gCLUDE_DAYS: exclude_days=$check->{exclude_days}, today=$day";
         return $ret;
      }
   }
   ...
   if (defined $check->{time_ranges}) {
      ...
      for my $pair (split /,/, $check->{time_ranges}) {
         ...
         if ($pair =~ /^(\d{2}:\d{2})-(\d{2}:\d{2})$/) {
            ...
            $ret->{stauts} = "ERROR: BAD_TIME_FORMAT in time_ranges at '$pair'. expecting HH:MM-HH:MM";
            return $ret;
         }
      }
         
      if (!$is_in_range) {
         $ret->{stauts} = "OK: NOT_IN_TIME_RANGE: time_ranges=$check->{time_ranges}, now=$HHMM_now";
         return $ret;
      }
   }
   ...
   if (defined $check->{hostnames}) {
      ...
      if (! grep {$hostname eq $_} @hostnames) {
         $ret->{stauts} = "OK: NOT_IN_HOSTNAMES: hostnames=$check->{hostnames}, localhost=$hostname";
         return $ret;
      }
   }
   my $tmpref;
   ...
   my $before_YYYYMMDD = sprintf("%04d%02d%02d", $year+1900, $mon+1, $mday);
   ...
   my $after_YYYYMMDD = sprintf("%04d%02d%02d", $year+1900, $mon+1, $mday);
   ...
   for my $venue (qw(Stdout Stderr)) {
      ...
      if (defined $check->{"Expected$venue"}) {
         for my $pattern (@{$check->{"Expected$venue"}}) {
            my @matches = grep {/$check->{"Expected$venue"}/} $tmpref->{$venue};
         
            if (@matches) {
               $match->{"Expected$venue"}->{match} = truncate_lines($maxchar, \@matches);
               $match->{"Expected$venue"}->{ok}    = "matched /" . $check->{"Expected$venue"} . "/";
            } else {
               $match->{"Expected$venue"}->{error} = "didn't match /" . $check->{"Expected$venue"} . "/";
            }
         }
      }
      ...
      if (defined $check->{"Unexpected$venue"}) {
         for my $pattern (@{$check->{"Unexpected$venue"}}) {
            my @matches = grep {/$check->{"Unexpected$venue"}/} $tmpref->{$venue};
         
            if (@matches) {
               $match->{"Unexpected$venue"}->{match} = truncate_lines($maxchar, \@matches);
               $match->{"Unexpected$venue"}->{error} = "matched /" . $check->{"Unexpected$venue"} . "/";
            } else {
               $match->{"Unexpected$venue"}->{ok} = "didn't match /" . $check->{"Unexpected$venue"} . "/";
            }
         }
      }
   }
   ...
   if (defined $check->{ExpectedRC}) {
      if ( "$rc" =~ /$check->{ExpectedRC}/) {   # TODO: use eval to implement expression instead
         $match->{ExpectedRC}->{match} = ["$rc"];
         $match->{ExpectedRC}->{ok} = "'$rc' matched /$check->{ExpectedRC}/";
      } else {
         $match->{ExpectedRC}->{error} = "'$rc' didn't match /$check->{ExpectedRC}/";
      }
   }
   ...
   if (defined $check->{UnexpectedRC}) {
      if ( "$rc" !~ /$check->{UnexpectedRC}/) {
         ...
         $match->{UnexpectedRC}->{ok} = "'$rc' didn't match /$check->{ExpectedRC}/";
      } else {
         $match->{UnexpectedRC}->{error} = "'$rc' matched /$check->{UnexpectedRC}/";
      }
   }
...
sub truncate_lines {
   ...
   for my $line (@$aref) {
      ...
      if ($total_length + $len < $maxchar) {
         ...
         push @truncated, substr($line, 0, $maxchar - $total_length) . "(truncated)";
         last;
      }
   }
