         ...
         if ( $char_count > $ItemChars || $line_count > $ItemLines ) {
            if ($ItemWarn) {
               if ( $char_count > $ItemChars ) {
                  carp
"item cut off here: char count ($char_count) > limit ($ItemChars).";
               } else {
                  carp
"item cut off here: line count ($line_count) > limit ($ItemLines).";
               }
            }
...
sub parse_PatternCfg {
   ...
   if ( $opt->{PatternFile} ) {
      ...
      croak
"Don't know where to find Pattern File. Noramlly \$TPSUP/scripts/log_pattern.cfg. Please set \$TPSUP in env.";
   }

   if ( !exists $PatternCfg_by_file->{$file} ) {
      croak "$file is not found" if !-f $file;

      $PatternCfg_by_file->{$file} =
        parse_simple_cfg( $file, { CfgKey => 'name', %$opt } );
   }
...
sub get_PatternCfg_by_app {
   ...
   if ( !defined($name) || $name eq 'auto' ) {
      ...
      croak "missing attr=LogFile in get_PatternCfg_by_app"
        if !defined $LogFile;
   ...
   if ( $cfg->{InSortKeyList}->{yyyy} ) {
      if ( $cfg->{assigned}->{yyyy} ) {
         ...
         # convert 21 to 2021
         $cfg->{func}->{yyyy} =
           sub { return sprintf( "20%02d", $_[0]->{yy} ); };
      } else {
   ...
   if ( $cfg->{InSortKeyList}->{mm} ) {
      if ( $cfg->{assigned}->{mm} ) {
         ...
         $cfg->{func}->{mm} =
           sub { return $TPSUP::LOG::mm_by_Mon{ $_[0]->{Mon} }; };
   ...
   $cfg->{func}->{HHMMSS} =
     sub { return $_[0]->{HH} . $_[0]->{MM} . $_[0]->{SS}; };
   ...
   my $KeyDelimiter =
     defined( $opt->{SortKeyDelimiter} ) ? $opt->{SortKeyDelimiter} : "";
...
# 'auto' is a reserved word to avoid deadloop, should not be used in config file.
      croak "'auto' is a reserved word, should not be used in config file"
        if $app eq 'auto';
   ...
   if ( $row_count > 0 ) {
      ...
      if ( defined $max_idx ) {
         ...
         if ( $max_percent < $threshold ) {
            print STDERR
"Tried to auto-detect pattern, but even the best-matching pattern has very low matching ratio $max_percent\%, < threshold $threshold\%\n";
...
sub yyyymmddHHMMSS_to_log_time {
   ...
   if ( $type eq 'ARRAY' ) {
      if ( @$yyyymmddHHMMSS == 2 ) {
         ...
         @{$r}{qw(yyyy mm dd)} =
           ( $r->{yyyymmdd} =~ /^(\d{4})(\d{2})(\d{2})$/ );
         croak "unknown format of yyyymmdd string = '$r->{yyyymmdd}'"
           if !defined $r->{dd};

         @{$r}{qw(HH   MM SS)} =
           ( $r->{HHMMSSSS} =~ /^(\d{2})(\d{2})(\d{2})$/ );
         croak "unknown format of HHMMSS string = '$r->{HHMMSS}'"
           if !defined $r->{SS};
         ...
         croak "unknown format of yyyymmddHHMMSS aref = "
           . Dumper($yyyymmddHHMMSS);
      }
   } else {
...
sub get_log_time {
   ...
   croak( "missing attr=compiled in cfg = " . Dumper($cfg) )
     if !$cfg->{compiled};
   ...
   if (@matches) {
      ...
      $r->{yyyymmdd} = $cfg->{func}->{yyyymmdd}->($r)
        if exists $cfg->{func}->{yyyymmdd};
   ...
   while (1) {
      ...
      if ($progress) {
         if ( $line_count % $progress == 0 ) {
            ...
            print STDERR
"$line_count lines are processed $direction, $progress lines in $seconds seconds\n";
            if ($verbose) {
      ...
      if ($backpattern) {
         ...
         $verbose >= 2
           && print STDERR "backward check: $key le $backstring ?\n";
         if ( $key le $backstring ) {
            ...
            if ($need_to_print_test_detail) {
               print STDERR
                 "last test pattern '$criteria' failed at line:\n$line";
            }
         }
         ...
         if ( $key gt $startstring ) {
            ...
            if ($need_to_print_test_detail) {
               print STDERR
                 "last test: $key gt $startstring, failed at line:\n$line";
            }
         }
      }
   }
   ...
   if ($progress) {
      ...
      $opt->{verbose}
        && print STDERR
"total $line_count lines are processed $direction in $seconds seconds\n";
   }
   ...
   if ( !defined($pos) ) {
      ...
      if ( defined $pos ) {
         print STDERR
"WARN: cannot find the backward position using $criteria. setting pos=$pos\n";
      } else {
...
sub get_log_latency {
   ...
   if ( $old->{yyyymmdd} && $new->{yyyymmdd} ) {
      $seconds =
        get_seconds_between_two_days( $old->{yyyymmdd}, $new->{yyyymmdd},
         $opt );
   }
   ...
   if ( defined $new->{MS} && defined $old->{MS} ) {
      $seconds =
        sprintf( "%.3f", $seconds + ( $new->{MS} - $old->{MS} ) * 0.001 );
   }
...
sub get_logname_cfg {
   ...
   my $string2 = resolve_scalar_var_in_string( $cfg_string,
      { yyyymmdd => $yyyymmdd, user => $user } );
...
sub get_logs_by_cfg {
   ...
   if ($BackwardDays) {
      for ( my $i = 1 ; $i <= $BackwardDays ; $i++ ) {
         my $day = `date -d "$yyyymmdd - $i day" "+%Y%m%d"`;
         chomp $day;
         push @days, $day;
      }
   }
...
#    https://stackoverflow.com/questions/25399728/perl-variable-will-not-stay-shared
   my $consume_line_update_item = sub {
      ...
      $line =
        undef; # after $line is consumed, reset it to undef to avoid double use.
   };
   ...
   return sub {
      ...
      return undef
        if $all_done;    # we have finished reading the file in last call
      ...
      while (@$logs) {
         ...
         if ($fh) {
            while ( defined( $line = <$fh> ) ) {
               next
                 if defined($CompiledPreMatch) && $line !~ /$CompiledPreMatch/;
               next
                 if defined($CompiledPreExclude)
                 && $line =~ /$CompiledPreExclude/;
        ...
        # next;
                  ...
                  ( $CompiledPostMatch && $line !~ /$CompiledPostMatch/ )
...
sub filter_log_section_gen {
   ...
   for my $ExpName (qw(MatchExp ExcludeExp)) {
      ...
      $compiledExp->{$ExpName} =
        eval_exp_to_sub( $exp, { ExpName => $ExpName } );
   }
   ...
   return sub {
      while ( my $r = $section_gen->() ) {
         next
           if defined($CompiledMatch)
           && !$CompiledMatch->( $r, { verbose => $verbose } );
         next
           if defined($CompiledExclude)
           && $CompiledExclude->( $r, { verbose => $verbose } );
...
EOF
      ...
      {
         my $section_gen = get_log_section_gen(
            $log, $cfg,
...
EOF
      ...
      {
         my $headers = get_log_section_headers(
            $cfg->{ExtractPatterns},
