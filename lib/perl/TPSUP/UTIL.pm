...
"ssh -n -o StrictHostKeyChecking=no -o ConnectTimeout=5 -o BatchMode=yes $host";
        ...
        if ( defined $SkipHead ) {
            ...
            $cmd =~ s/cat $path|sed 1,${SkipHead}d/sed 1,${SkipHead}d $path/
              ;                                                       # optimize
        }
...
sub get_patterns_from_log {
    ...
    my $EndPattern =
      defined $opt->{EndPattern} ? qr/$opt->{EndPattern}/ : undef;
    ...
    for my $string (@strings) {
        ...
        for my $pair (@$pairs) {
            if ( $pair =~ /^(.+?)=(.+)/ ) {
                ...
                croak
                  "bad Exp pair: '$pair'. Expecting 'key=exp'. from '$string'";
            }
        }
    }
        ...
        if ( $pair =~ /^(.+?)=(.+)/s ) {
            ...
            for my $ns ( split /,/, $namespaces ) {
                if ( $type eq 'file' ) {
                    ...
                    croak
                      "unknowtype='$type' when calling insert_namespaces().";
                }
            }
        }
        else {
...
"max_level=$max_level reached when trying to resolve clause=$clause. use verbose mode to debug"
    ...
    # recursive call
    $clause = resolve_scalar_var_in_string( $clause, $Dict,
        { %$opt, level => $level } );
