...
sub parse_fix_message {
   ...
   if ($opt->{FixDelimiter}) {
      ...
      if ($line =~ /\b8=FIX[.0-9]+?([^.0-9]+)[0-9]/ || $line =~ /\b35=[0-9A-Z]{1,2}([^.0-9A-Z]+)[0-9]+=/ ) {
         $delimiter = $1;
         if ($delimiter eq '^A') {
            ...
            $line =~ s/^A//g;
            $delimiter = '';
            $opt->{verbose} && print "converted delimiter from '^A' (^+A) to '' (Ctrl+A)\n";
         }
      } elsif ($line =~ /^\s*\d+=[^=]+$/) {
   ...
   my $is_New_Multileg; # http://www.onixs.biz/fix-dictionary/4.4/msgType_AB_6566.html
...
sub get_order_hierarchy {
   ...
   if ($opt->{FixPickOrdFile}) {
      ...
      $opt->{verbose} && print STDERR "pick_this_ord = ", Dumper($pick_this_ord);
   }
   ...
   while (<$in_fh>) {
      ...
      if ($opt->{ShowProgress}) {
         if ($line_count % $opt->{ShowProgress} == 0 ) {
            ...
            print STDERR "$line_count lines are processed. $opt->{ShowProgress} lines in $seconds seconds\n";
      ...
      my $parsed = parse_fix_message($line, {%$opt, ReturnNestedInArray=>1});
   ...
   if ($opt->{ShowProgress}) {
      ...
      print STDERR "Total $line_count lines are processed in $total_sec seconds.\n";
   }
   ...
   for my $id (keys %$exists_ord) {
      ...
      while (exists $ord_by_orig->{$current}) {
         ...
         delete $ord_by_orig->{$current}; # delete the link to ensure we only use it once
   ...
   if ($opt->{FixHierArchyOutput}) {
      print_csv_hashArray($ret->{array}, $ret->{columns},
                            {
                               %$opt,
...
sub filter_fix {
   ...
   if ($opt->{MatchExps} && @{$opt->{MatchExps}}) {
      @$matchExps = map { TPSUP::Expression::compile_exp($_, $fixopt) } @{$opt->{MatchExps}};
   }

   my $excludeExps;
   if ($opt->{ExcludeExps} && @{$opt->{ExcludeExps}}) {
      @$excludeExps = map { TPSUP::Expression::compile_exp($_, $fixopt) } @{$opt->{ExcludeExps}};
   }
   ...
   my @exportTags = $expcfg->{ExportExps}->{Cols} ? @{$expcfg->{ExportExps}->{Cols}} : ();

   my $tempExps = $expcfg->{TempExps}->{Exps};
   my @tempTags = $expcfg->{TempExps}->{Cols} ? @{$expcfg->{TempExps}->{Cols}} : ();
   ...
   while (<$in_fh>) {
         ...
         if ($line =~ /$NewOrder_qr/) {
            ...
            for my $r (@splits) {
               ...
               if ($tag35 eq 'D') {
                  ...
                  if ( defined($old_OrdID) && defined($OrderInfo->{$old_OrdID}) ) {
                     %{$OrderInfo->{$OrdID}} = (%{$OrderInfo->{$old_OrdID}}, %$r);
                  }
               } elsif ($tag35 eq 'F') {
                  ...
                  if ( defined($old_OrdID) && defined($OrderInfo->{$old_OrdID}) ) {
                     %{$OrderInfo->{$OrdID}} = (%{$OrderInfo->{$old_OrdID}}, {11=>$OrdID});
                  }
               } elsif ($tag35 eq 'AB') {
                  ...
                  if ( defined($LegID) ) {
                     ...
                     carp "New Multileg order: 35=$tag35,11=$OrdID,has no leg (tag 654)";
                  ...
                  if ( defined($old_OrdID) && defined($OrderInfo->{$old_OrdID}) ) {
                     ...
                     carp "Multileg replace: 35=$tag35,11=$OrdID,41='$old_OrdID'. cannot find old order by tag 41";
                     dump_fix_message($line);
                  }
               } elsif ($tag35 eq 'E') {
                  ...
                  if ( defined($ListID) ) {
                     ...
                     carp "New List order: 35=$tag35,11=$OrdID,has no ListID (tag 66)";
                     dump_fix_message($line);
                  }
               } 
            }
      ...
      if ($opt->{GenFixMsg}) {
         ...
         my $ref = parse_fix_message($line, {%$opt, ReturnNestedInArray=>1});
      ...
      my @to_be_assembled; #GenFixMsg will need this to re-assemble the output message
      ...
      for my $r (@splits) {
         if ($FilterExists) {
            ...
            for my $FilterTag (@FilterTags) {
               ...
               next if ! defined $value; # this log line doesn't contain this tag at all
         
               next if ! $FilterExists->{$value}; #this log line contain this tag but doesn't
         ...
         if ($matchExps || $excludeExps || $exportExps) {
               ...
               if (defined($OrdID)) {
                  if (defined($LegID) && $tag35 ne 'AB' && $tag35 ne 'AC') {
                     ...
                     if (!defined $old_info && !$opt->{SourceNewOrderQuiet}) {
                        carp "cannot find Multileg order entry for 11=$OrdID,654=$LegID";
                     }
            ...
            if (defined $old_info) {
               ...
               TPSUP::Expression::export_var($combined, {FIX=>1, RESET=>1});
            } else {
            ...
            if (@tempTags) {
               ...
               TPSUP::Expression::export_var($temp_r, {FIX=>1}); # don't RESET here
            }
      ...
      if ($opt->{GenFixMsg}) {
         ...
         my $output_delimiter = $opt->{GenFixDelimiter} ? $opt->{GenFixDelimiter} :
                                                          $input_delimiter        ;
         ...
         if (@to_be_assembled != @splits) {
            ...
            { 
               ...
               $opt2->{FixFrontTags} = $opt->{GenFixFrontTags} ? $opt->{GenFixFrontTags} :
                                                                     $detail->{CommTags} ;
            ...
            for (my $i=0; $i<@to_be_assembled; $i++) {
               next if !$detail->{CompTagMx}->[$i] || !@{$detail->{CompTagMx}->[$i]};
               my $opt2 = {
                  %$opt,
                  FixDeleteTags => $opt->{GenFixDeleteTags},
                  FixDeleteExp  => $opt->{GenFixDeleteExp},
      ...
      if ( $opt->{FilterFixMaxMatch} ) {
         if (      $opt->{MatchExps}     || $opt->{ExcludeExps}
             || $opt->{MatchPatterns} || $opt->{ExcludePatterns} ) {
   ...
   if ($opt->{RenderOutput} || $PrintCsv) {
      print_csv_hashArray($ref2->{array}, $ref2->{columns},
                           {
                             %$opt,
...
sub hash_to_fix {
   ...
   if ($opt->{FixDeleteExp}) {
      ...
      if (!$DeleteExp_by_string->{$string}) {
         ...
         my $compiled = eval "$warn warnings; no strict; package TPSUP::Expression; sub {my (\$tag, \$fix)=\@_; $converted_string}";

         croak "Bad DeleteExp '$converted_string': $@. converted from $string" if $@;
   ...
   sub test_DeleteExp {
      ...
      TPSUP::Expression::export_var($r, {FIX=>1}); # no RESET here, only overwrite the tag
...
sub csv_to_fix {
...

   {
      ...
      $opt->{verbose} && print "in csv_to_fix() before query_csv2(), csv_opt = ", Dumper($csv_opt);
   ...
   my $delimiter = defined $opt->{CSV2FIX_delimiter} ? $opt->{CSV2FIX_delimiter} : '';
   my $fix_protocol = $opt->{CSV2FIX_Protocol} ? $opt->{CSV2FIX_Protocol} : '4.2';
   ...
   for my $r ( @{$ref->{array}} ) {
      ...
      $message .= hash_to_fix($r, {%$opt, delimiter=>$delimiter, tags=>$ref->{columns}} );
                  ...
                  if ($size < $width) {
                     ...
                     if ($desc) {
                        if (length($desc) > $width-$size-3) {
                           my $sub_desc = substr($desc, 0, $width-$size-3);
                           $string .= " ($sub_desc)";
                        } else {
                           $string .= " ($desc)";
                        }
