...
sub explore_xml {
      ...
      && @{ $opt->{HandleActs} } )
   {
      ...
      $opt2->{Handlers} = transpose_arrays(
      ...
      && @{ $opt->{FlowDirs} } )
   {
      ...
      $opt2->{FlowControl} =
        transpose_arrays( [ $exps, $dirs, $opt->{FlowExps}, $opt->{FlowDirs} ],
         $opt );
   }
...
sub query_xml {
   ...
   if ( $opt->{KeyAttr} ) {
      ...
      if ( $type eq 'ARRAY' ) {
         ...
         croak
"KeyAttr has unsupport ref type='$type'. only ref to ARRAY or HASH are supported.";
      }
   } else {
      $KeyAttr = [];
   }
...
# therefore, we allow user to use r->{body} and then convert it to $r->{body} here
      ...
      my $compiled =
        eval "$warn warnings; no strict; sub { my \$r=shift; \$root=\$r; $p } ";
